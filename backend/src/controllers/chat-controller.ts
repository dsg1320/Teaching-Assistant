...
const allowedComplexities = ["easy", "moderate", "difficult"] as const;
type Complexity = typeof allowedComplexities[number];

// Function to calculate complexity
const calculateComplexity = (message: string): Complexity | null => {
  if (message.includes('bubble sort')) return 'easy';
  if (message.includes('merge sort')) return 'moderate';
  if (message.includes('heap sort')) return 'difficult';
  return null;
};
